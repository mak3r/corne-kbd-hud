...
The HUD is also movable (click and drag it anywhere; the position is remembered across restarts) and highlights each key with a white outline while it's physically held down, driven by the firmware's per-keystroke broadcast (see `halcyon-corne`'s `hud_console.c`) — both confirmed working in the packaged `.app`.
